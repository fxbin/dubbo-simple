...
public final class Result<T> implements Serializable {
    ...
    private Result(ResultCode resultCode) {
        this(resultCode.code, resultCode.errmsg);
    }

    private Result(ResultCode resultCode, T data) {
        this(resultCode.code, resultCode.errmsg, data);
    }
    ...
    public static String getErrMsg(@Nullable Result<?> result) {
        return Optional.ofNullable(result).isPresent() ? Optional.of(result.errmsg).get() : ResultCode.FAILURE.errmsg;
    }
     ...
     * failure
     *
     * @since 2021/7/4 11:40
     * @param ex cn.fxbin.framework.dubbo.exception.ServiceException
     * @return cn.fxbin.framework.dubbo.model.Result<T>
     */
    public static <T> Result<T> failure(ServiceException ex) {
        return failure(ex.getErrcode(), ex.getErrmsg());
    }

    /**
     * failure
     *
     * @since 2021/7/4 11:41
     * @param ex cn.fxbin.framework.dubbo.exception.GlobalException
     * @return cn.fxbin.framework.dubbo.model.Result<T>
     */
    public static <T> Result<T> failure(GlobalException ex) {
        return failure(ex.getErrcode(), ex.getErrmsg());
    }
